...
module.exports = function videoCut() {
	let that = null;
	 ...
	 * @param  {string} spec.startTime Start time of the cut in `hh:mm:ss` format.
	 * @param  {string} spec.duration Duration of the cut in `hh:mm:ss` format.
	 * @param  {string} spec.fileName Filename of the video to be cut
	 * @return {Promise} A promise for an object containing startTime, duration, and fileName of the cut clip.
	 */
	function cutVideo(spec) {
	  ...
	  return new Promise((resolve, reject) => {
	    ...
	    let commandQuery = 
	      `ffmpeg -i ${spec.fileName} -vcodec copy -acodec copy -ss ${spec.startTime} -t ${spec.duration} ${cutFileName} -y`;
	    
	    let child = shelljs.exec(commandQuery, { async: true, silent: true });
